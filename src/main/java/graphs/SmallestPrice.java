...
public class SmallestPrice {
    ...
    public static int getSmallestPrice(WeightedGraph graph, int source, int maxTime, List<Pair> destinations) {
        ...
        for ( Pair dest : destinations){
            destMap.put(dest.node,dest.price);
        }
        distances[source] = 0;
        ...
        while (!queue.isEmpty()){
            int u = queue.poll();
            Integer val = destMap.get(u);
            if ( val != null){
                min = Math.min(min, val); // check the price at each iter
                destMap.remove(u);
            }
            if (destMap.isEmpty()){
                break;
            ...
            for (DirectedEdge e : graph.adj[u]){
                ...
                if ( newdist < distances[e.w] && newdist <= maxTime){ // u is the current smallest path to get to w
