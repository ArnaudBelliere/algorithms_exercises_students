...
public class SantaInventory {
    ...
    public void put(int toyPrice, int count) {
        Node new_node = new Node(toyPrice,count);
        root = put(root,new_node);

    }
    private Node put(Node node,Node new_node){

        if (node == null){
            return new_node;
        }
        if (node.toyPrice < new_node.getToyPrice()){
            node.right = put(node.right,new_node);
        }
        else if (node.toyPrice > new_node.getToyPrice()){
            node.left = put(node.left,new_node);
        }
        else {
            node.setCount(node.getCount()+ new_node.count);
        }
        node.incSize(new_node.count);
    ...
    public int median() {
        ...
        int median_index = (root.getSize()/2)+1;

        return median(root,median_index);
    }
    private int median(Node node, int median_index){
        int lsize = node.left == null ? 0 : node.left.size ;

        if (median_index <= lsize){
            return median(node.left,median_index);
        }
        if (median_index <= lsize + node.count){
            return node.toyPrice;
        }
        return median(node.right, median_index-lsize-node.count);
    }
